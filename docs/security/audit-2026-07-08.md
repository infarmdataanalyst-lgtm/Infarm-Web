...
- **Perbaikan:** `orders/create` kini ambil ulang harga tiap produk dari DB (`promo_price`), hitung `subtotal` + `total` (= subtotal + ongkir − diskon) server-side; harga & total dari client diabaikan; produk tak ada/diarsipkan ditolak (422). Terverifikasi: kirim `price:1/totalAmount:1` untuk produk Rp75.000 → tersimpan Rp83.400 (harga DB + ongkir).
