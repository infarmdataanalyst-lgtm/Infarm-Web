...
// Menitipkan penanda GA4 (client_id & session_id) ke pesanan yang BARU tersimpan.
//
...
//
// Keduanya ditulis dalam SATU update, bukan dua: mereka selalu dibaca bersama oleh webhook, dan
// menulisnya terpisah membuka keadaan setengah jadi yang tak ada gunanya.
//
...
export async function attachGaIdentifiers(
  invoice: string,
  ids: { clientId?: string; sessionId?: string },
): Promise<boolean> {
  // `session_id` bisa terbaca tanpa `client_id` (mis. cookie `_ga` diblokir tapi `_ga_<id>`
  // tidak), jadi keduanya opsional dan yang kosong tidak ikut ditulis — jangan menimpa nilai yang
  // sudah ada dengan null.
  const patch: Record<string, string> = {}
  if (ids.clientId) patch.ga_client_id = ids.clientId
  if (ids.sessionId) patch.ga_session_id = ids.sessionId
  if (Object.keys(patch).length === 0) return true

  const supabase = createAdminClient()
  const { error } = await supabase.from('orders').update(patch).eq('nomor_invoice', invoice)
